...
function IotDevices(props) {
   ...
   const handleFilter = (evKey) => {
      ...
      */
      const newFilteredDevices = optionSelected!=='All' ? allIotDevices.allData.filter(device=>{
            if (formattedFloorName(device.floor)===optionSelected)
               return device;
         }   
   ...
   const loadIotDevices = async () => {
      ...
      const makeRequest = () => {
         return iotDeviceUtils.fetchIotDevices(
            currentPortfolio,
            userToken
         );
      };
      const res = iotDeviceUtils.handleFilterLoadingAsync(
         makeRequest,
   ...
   let devicesToShow = allIotDevices.filterSelected!==false ? allIotDevices.filteredData : allIotDevices.allData;
   return (
      <>
